...
test.describe('Journal Entry by ID API', () => {
	test.describe('GET /api/journal/{entryId}', () => {
		...
		test('[JOURNAL-API-014] should return 404 for incorrect entry ID', async ({request}) => {
			const fakeId = '12345'
		...
		test('[JOURNAL-API-015] should reject unauthenticated GET request', async ({ request }) => {
	...
	test.describe('PATCH /api/journal/{entryId}', () => {
		test('[JOURNAL-API-016] should update a journal entry', async ({ request }) => {
		...
		test('[JOURNAL-API-017] should partially update a journal entry', async ({ request }) => {
			// Create a new entry first
			const newEntry = {
				date: new Date().toISOString().split('T')[0],
				text: 'Original text for partial PATCH test',
				tag: 'original-tag'
			}
			...
			expect(getData.entry.tag).toBe(newEntry.tag) // Should remain unchanged

			// Cleanup
			await request.delete(`/api/journal/${entryId}`)
		})

		test('[JOURNAL-API-018] should reject PATCH with invalid date format', async ({request}) => {
		...
		test('[JOURNAL-API-019] should return 404 when updating non-existent entry', async ({request}) => {
		...
		test('[JOURNAL-API-020] should reject unauthenticated PATCH request', async ({request}) => {
			// Create an entry first
			const newEntry = {
				date: new Date().toISOString().split('T')[0],
				text: 'Entry for auth test',
	...
	test.describe('DELETE /api/journal/{entryId}', () => {
		test('[JOURNAL-API-021] should delete a journal entry', async ({ request }) => {
		...
		test('[JOURNAL-API-022] should return 404 when deleting non-existent entry', async ({request}) => {
			const fakeId = 'non-existent-id-99999'
			const response = await request.delete(`/api/journal/${fakeId}`)
			expect(response.status()).toBe(404)
		})

		test('[JOURNAL-API-023] should reject unauthenticated DELETE request', async ({request}) => {
			// Create an entry first
			const newEntry = {
				date: new Date().toISOString().split('T')[0],
				text: 'Entry for auth test',

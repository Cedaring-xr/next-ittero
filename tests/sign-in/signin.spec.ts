...
test('should sign-in to the test account using email and password', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'email' }).fill('mattray.maps@gmail.com')
    // input the password for test account into the password field
    await page.getByRole('textbox', {name: 'password'}).fill('trick1190@Node')
    // click the log-in button
    await page.click('test=Log In')
